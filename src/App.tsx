...
function saveToStorage(records: WorkoutRecord[]): void {
  try {
...
function App() {
  ...
  useEffect(() => {
    ...
    setRecords(data);
    if (data.length > 0) {
      const exercises: string[] = [...new Set(data.map((r: WorkoutRecord) => r.exercise))];
  ...
  const handleBodyPartChange = (bp: string): void => {
    const exercises = BODY_PARTS[bp];
    setForm({ ...form, bodyPart: bp, exercise: exercises[0] });
  };

  const handleSubmit = (): void => {
    if (!form.weight || !form.reps || !form.sets) {
      showToast("⚠️ 請填寫所有欄位");
      return;
    }
  ...
  const handleEdit = (r: WorkoutRecord): void => {
    setForm({
      date: r.date,
      bodyPart: r.bodyPart,
      exercise: r.exercise,
  ...
  const filteredRecords = useMemo<WorkoutRecord[]>(() => {
    ...
    if (filterPart === "全部") return sorted;
    return sorted.filter((r: WorkoutRecord) => r.bodyPart === filterPart);
  }, [records, filterPart]);
  ...
  const progressData = useMemo<ProgressPoint[]>(() => {
    return records
      ...
      .map((r: WorkoutRecord): ProgressPoint => ({
  ...
  const weeklyVolume = useMemo<VolumeItem[]>(() => {
    ...
    return Object.entries(vol).map(
      ([name, volume]: [string, number]): VolumeItem => ({ name, volume })
    );
  }, [records]);

  const stats = useMemo(() => {
    const totalVolume = records.reduce(
      (s: number, r: WorkoutRecord) => s + r.weight * r.reps * r.sets, 0
    );
    return {
  ...
  return (
    ...
    >
      ...
      {toast !== null && (
        <div
          style={{
            ...
            zIndex: 999, background: "rgba(20,20,40,0.95)",
            border: "1px solid rgba(99,102,241,0.3)", padding: "12px 24px",
            borderRadius: 12, backdropFilter: "blur(20px)", fontSize: 14,
            fontWeight: 500, color: "#c4b5fd", animation: "slideDown 0.3s ease",
          }}
        >
          {toast}
        </div>
      ...
      <div
        style={{
          maxWidth: 520, margin: "0 auto", padding: "0 16px 100px",
        ...
        <div style={{ padding: "32px 0 20px", textAlign: "center" }}>
          <div
            style={{
              ...
              color: "#6366f1", fontWeight: 600, marginBottom: 8,
            }}
          >
            Workout Tracker
          </div>
          ...
          <div
            style={{
              fontSize: 11, color: "#4ade80", marginTop: 6, display: "flex",
            ...
            {statItems.map((s: StatItem, i: number) => (
              ...
              >
                <div style={{ fontSize: 20, fontWeight: 800, color: "#e0e7ff" }}>
                  {s.value}
                  <span style={{ fontSize: 11, fontWeight: 400, color: "#818cf8", marginLeft: 2 }}>
                    {s.unit}
                  </span>
                </div>
                <div
                  style={{
                    fontSize: 10, color: "#6366f1", letterSpacing: 1, marginTop: 2,
          ...
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                <span style={{ fontSize: 18 }}>{editId !== null ? "✏️" : "🏋️"}</span>
                <span style={{ fontSize: 16, fontWeight: 700, color: "#e0e7ff" }}>
                  {editId !== null ? "編輯記錄" : "新增記錄"}
                </span>
              </div>
        ...
        {activeTab === "history" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            {Object.entries(groupedByDate).map(
              ([date, recs]: [string, WorkoutRecord[]]) => (
            ...
            {progressData.length < 2 ? (
              ...
              <>
                ...
                <div style={chartCardStyle}>
                  <div
                    style={{ fontSize: 14, fontWeight: 700, color: "#e0e7ff", marginBottom: 16 }}
                  >
                    ⚖️ 重量趨勢 (kg)
                  </div>
                    ...
                    <LineChart data={progressData}>
                      ...
                      <Tooltip
                        contentStyle={{
                          ...
                          borderRadius: 10, fontSize: 12, color: "#e0e7ff",
                        }}
                      />
                      <Line
                        type="monotone"
                  ...
                  {(() => {
                    ...
                    return (
                      <div
                        style={{
                          ...
                          background: diff > 0
                            ...
                            : "rgba(239,68,68,0.08)",
                          border: `1px solid ${
                            diff > 0 ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"
                          }`,
                          fontSize: 12,
                      ...
                      >
                        {diff > 0 ? "📈" : "📉"} 重量
                        {diff > 0 ? "增加" : "減少"}咗 {Math.abs(diff)}kg（{first}kg →{" "}
                        {last}kg）
                      </div>
                    );
                  })()}
